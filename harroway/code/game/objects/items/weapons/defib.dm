...
/obj/item/weapon/material/twohanded/defibrillator/New()
	sparks.set_up(5,0,src)
	sparks.attach(src)
	return ..()
/*
/obj/item/weapon/material/twohanded/defibrillator/suicide_act(mob/user)
	...
	playsound(get_turf(src),'harroway/sound/machines/defib.ogg',50,1)
	return (FIRELOSS)
*/
/obj/item/weapon/material/twohanded/defibrillator/update_icon()
	icon_state = "defib"
	if(ready)
...
/obj/item/weapon/material/twohanded/defibrillator/attack_self(mob/user)
	if(charges || ready)
...
/obj/item/weapon/material/twohanded/defibrillator/update_twohanding(mob/user)
	..()
	item_state = "defib[wielded ? 1 : 0]"
	force = wielded ? 40 : 10
	if(user)
		user.update_inv_l_hand()
		user.update_inv_r_hand()

/obj/item/weapon/material/twohanded/defibrillator/attackby(obj/item/weapon/W,mob/user)
...
/obj/item/weapon/material/twohanded/defibrillator/attack(mob/M,mob/user)
	if(!ishuman(M))
...
/obj/item/weapon/material/twohanded/defibrillator/proc/shockAttack(mob/living/carbon/human/target,mob/user)
...
/obj/item/weapon/material/twohanded/defibrillator/proc/attemptDefib(mob/living/carbon/human/target,mob/user)
	...
	if(do_after(user,30,target))
		...
		if(!head || head.status & ORGAN_DESTROYED || NOCLONE in target.mutations  || !target.has_brain() /*|| target.suiciding == 1*/)
		...
		if(target.mind && !target.client) //Let's call up the ghost! Also, bodies with clients only, thank you.
			var/mob/observer/ghost/ghost = get_ghost_from_mind(target.mind)
			if(ghost && ghost.client && ghost.admin_ghosted)
		...
		return
/*
/obj/item/weapon/material/twohanded/defibrillator/restock()
